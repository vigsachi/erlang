...
from training.linear_regression.train_ernest import *
from utils import cluster_utils, kube_utils, hpa_utils
import argparse
...
class ErnestAutoscaler(object):
    ...
    def run(self, num_iters=100000, wait_time=5):
    ...
    def get_pod_config(self, num_samples=20000):
        ...
        self.model.train_config.train_rps = [rps_interp]
        training_points = [sample_round_multivariate(self.bounds, self.model.sample_interval) for i in range(num_samples)]

        best_reward, best_training_point = None, None
        for training_point in training_points:
            training_point = [rps_interp] + training_point # Overwrite rps in sample with observed one.
            reward = self.predict_ols(training_point)
        ...
        for i, num_replicas in enumerate(best_training_point[1:]):
            ...
            num_replicas = num_replicas*self.model.sample_interval
    ...
    def build_ols_predictor(self):
        ...
        # Update latency threshold and lat opt.
        self.model.lg.lat_opt = 'Average Latency'
        self.model.lg.latency_threshold = 50

        # Process results from training for model fitting.
        print("Fitting a model with ", len(self.model.res), " points")
        labels, data_points = [], []
        for training_example in self.model.res:
            #labels.append(training_example['stats'][self.model.train_config.lat_opt]) # Latency
            labels.append(self.model.lg.compute_qoe(training_example['stats'], sum(training_example['training_point'][1:])))
            data_points.append(_get_features(training_example['training_point']))

        # Fit Ordinary Least Squares model.
        self.model.ols_model = LinearRegression()
        self.model.ols_model.fit(data_points, labels)
        #embed()
    ...
    def construct_search_space(self):
        ...
        self.bounds = []
        for deployment in self.model.search_coords:
            self.bounds.append([1, np.int(np.ceil(self.app_config.deployments[deployment] / self.model.sample_interval))] )
    ...
    def predict_ols(self, training_point):
        reward = self.model.ols_model.predict([_get_features(training_point)])
...
def sample_round_multivariate(bounds, sample_interval):

    sample = []
    for bound in bounds:
        sample.append(np.random.randint(bound[0],bound[1],1)[0] / sample_interval )
...
if __name__ == "__main__":
    ...
    ea = ErnestAutoscaler(args.train_config_path, args.pod_filter, args.init_nodes, args.add_nodes)
    ea.run()
