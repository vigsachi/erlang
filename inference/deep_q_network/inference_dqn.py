...
class DQNAutoscaler(object):
    ...
    def load_model(self):
        ...
        self.model.agent.is_training = False
    ...
    def get_pod_config(self):
        ...
        # Get currrent state and query for action.
        observation = self.model.get_state(rps_interp)
        action = self.model.agent.select_action(observation)
        ...
        for i, num_replicas in enumerate(action):
            ...
            deployment_replicas = (num_replicas + 1.0) * self.app_config.deployments[service] / 2 # Convert from [-1,1] to [0, max_rep]
            deployment_replicas = np.round(deployment_replicas)
            deployment_replicas = min(deployment_replicas, self.app_config.deployments[service])
            deployment_replicas = max(deployment_replicas, 1)

            scale_config[service] = int(deployment_replicas)
...
if __name__ == "__main__":
    ...
    dqn = DQNAutoscaler(args.train_config_path, args.pod_filter, args.init_nodes, args.add_nodes)
    dqn.run()
